...
func (p *Validator) ValidateX(b []byte) error {
	// Use flag to indicate it is not the last, for reader set it to eof or even empy buf
	return p.validateBytes(b, false)
}
...
func (p *Validator) ValidateReaderX(r io.Reader) error {
	return p.validate(nil, r)
}
...
func (p *Validator) validate(buf []byte, r io.Reader) error {
	if cap(p.stack) < stackMinSize {
		p.stack = make([]byte, 0, stackMinSize)
	} else {
		p.stack = p.stack[:0]
...
func (p *Validator) validateBytes(buf []byte, cont bool) error {
	if cap(p.stack) < stackMinSize {
		p.stack = make([]byte, 0, stackMinSize)
	} else {
		p.stack = p.stack[:0]
