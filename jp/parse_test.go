...
func TestParse(t *testing.T) {
	...
	for i, d := range []xdata{
		...
		{src: "abc[:-1]", expect: "abc[:-1]"},
		{src: "$['abc']", expect: "$.abc"},
		{src: "$['cognito:username']", expect: "$['cognito:username']"},
		{src: "$['a:b:c']", expect: "$['a:b:c']"},
		{src: "$[':']", expect: "$[':']"},
		...
		{src: "$.cognito:username", err: "parse error at 10 in $.cognito:username"},
		{src: "abc..cognito:username", err: "parse error at 13 in abc..cognito:username"},
		{src: "$.:foo", err: "an expression fragment can not start with a ':' at 4 in $.:foo"},
		{src: "$.a:b.c", err: "parse error at 4 in $.a:b.c"},
